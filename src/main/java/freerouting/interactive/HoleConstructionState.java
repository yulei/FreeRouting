...
public class HoleConstructionState extends CornerItemConstructionState
{
    ...
    private boolean start_ok(FloatPoint p_location)
    {
        ...
        java.util.Collection<freerouting.board.Item> found_items = hdlg.get_routing_board().pick_items(pick_location,
        ...
        freerouting.board.Item found_item = found_items.iterator().next();
        ...
        if (construction_succeeded)
        {
            ...
            else
            {
                ...
                hdlg.get_routing_board().insert_obstacle(new_obs_area, item_to_modify.get_layer(),
                        item_to_modify.clearance_class_no(), freerouting.board.FixedState.UNFIXED);
                if (this.observers_activated)
                {
