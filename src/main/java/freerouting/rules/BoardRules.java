...
public class BoardRules implements java.io.Serializable
{
     ...
     */
    public BoardRules(freerouting.board.LayerStructure p_layer_structure, ClearanceMatrix p_clearance_matrix)
    {
        layer_structure = p_layer_structure;
        clearance_matrix = p_clearance_matrix;
        nets = new Nets();
        this.trace_angle_restriction = freerouting.board.AngleRestriction.FORTYFIVE_DEGREE;
    ...
    public void create_default_via_rule(NetClass p_net_class, String p_name)
    {
        ...
        for (int i = 0; i < this.via_infos.count(); ++i)
        {
            ...
            {
                freerouting.library.Padstack curr_padstack = curr_via_info.get_padstack();
     ...
     * to p_to_no.
     */
    public void change_clearance_class_no(int p_from_no, int p_to_no, java.util.Collection<freerouting.board.Item> p_board_items)
    {
        for(freerouting.board.Item curr_item : p_board_items)
        {
        ...
        {
            freerouting.rules.NetClass curr_net_class = this.net_classes.get(i);
        ...
        for (int i = 0; i < this.via_infos.count(); ++i)
        {
            freerouting.rules.ViaInfo curr_via = this.via_infos.get(i);
     ...
     */
    public boolean remove_clearance_class(int p_index, java.util.Collection<freerouting.board.Item> p_board_items)
    {
        for(freerouting.board.Item curr_item : p_board_items)
        {
    ...
    public freerouting.board.AngleRestriction get_trace_angle_restriction()
    {
        return this.trace_angle_restriction;
    }
    
    /** Sets the angle restriction for tracese: 90 degree, 45 degree or none. */
    public void set_trace_angle_restriction(freerouting.board.AngleRestriction p_angle_restriction)
    {
    ...
    public double get_default_via_diameter()
    {
        ...
        freerouting.library.Padstack via_padstack = default_via_rule.get_via(0).get_padstack();
    ...
    throws java.io.IOException, java.lang.ClassNotFoundException
    {
        ...
        this.trace_angle_restriction = freerouting.board.AngleRestriction.arr[snap_angle_no];
    }
    ...
    private transient freerouting.board.AngleRestriction trace_angle_restriction;
