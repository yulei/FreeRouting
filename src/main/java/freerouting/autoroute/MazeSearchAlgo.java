...
public class MazeSearchAlgo
{
    ...
    private boolean expand_to_room_doors(MazeListElement p_list_element)
    {
        ...
        if (this.ctrl.with_neckdown && p_list_element.door instanceof TargetItemExpansionDoor)
        {
            ...
            if (start_item instanceof freerouting.board.Pin)
            {
                double neckdown_half_width = ((freerouting.board.Pin) start_item).get_trace_neckdown_halfwidth(layer_no);
        ...
        else if (p_list_element.next_room instanceof ObstacleExpansionRoom)
        {
            ...
            if (!p_list_element.already_checked)
            {
                ...
                if (ripup_costs != ALREADY_RIPPED_COSTS && next_room_is_thick)
                {
                    ...
                    if (!curr_door_is_small && this.ctrl.max_shove_trace_recursion_depth > 0 && obstacle_item instanceof freerouting.board.PolylineTrace)
                    {
        ...
        if (ctrl.vias_allowed && !(p_list_element.door instanceof ExpansionDrill))
        {
            ...
            else if (p_list_element.next_room instanceof ObstacleExpansionRoom)
            {
                ...
                if (curr_obstacle_item instanceof freerouting.board.Via)
                {
                    freerouting.board.Via curr_via = (freerouting.board.Via) curr_obstacle_item;
    ...
    private boolean door_is_small(ExpansionDoor p_door, double p_trace_width)
    {
        if (p_door.dimension == 1 || p_door.first_room instanceof CompleteFreeSpaceExpansionRoom && p_door.second_room instanceof CompleteFreeSpaceExpansionRoom)
        {
            ...
            {
                if (this.autoroute_engine.board.get_test_level().ordinal() >= freerouting.board.TestLevel.ALL_DEBUGGING_OUTPUT.ordinal())
                {
    ...
    private void expand_to_other_layers(MazeListElement p_list_element)
    {
        ...
        if (curr_drill.room_arr[p_list_element.section_no_of_door] instanceof ObstacleExpansionRoom)
        {
            ...
            freerouting.library.Padstack curr_obstacle_padstack = ((freerouting.board.Via) curr_obstacle_item).get_padstack();
    ...
    private static void reduce_trace_shapes_at_tie_pins(Collection<Item> p_item_list, int p_own_net_no, ShapeSearchTree p_autoroute_tree)
    {
        ...
        {
            if ((curr_item instanceof freerouting.board.Pin) && curr_item.net_count() > 1)
            {
                Collection<Item> pin_contacts = curr_item.get_normal_contacts();
                freerouting.board.Pin curr_tie_pin = (freerouting.board.Pin) curr_item;
                for (Item curr_contact : pin_contacts)
                {
                    if (!(curr_contact instanceof freerouting.board.PolylineTrace) || curr_contact.contains_net(p_own_net_no))
                    {
                        continue;
                    }
                    p_autoroute_tree.reduce_trace_shape_at_tie_pin(curr_tie_pin, (freerouting.board.PolylineTrace) curr_contact);
                }
            }
        }
    }
    ...
    private boolean room_shape_is_thick(ObstacleExpansionRoom p_obstacle_room)
    {
        ...
        if (obstacle_item instanceof freerouting.board.Trace)
        {
            obstacle_half_width = ((freerouting.board.Trace) obstacle_item).get_half_width()
                    + this.search_tree.clearance_compensation_value(obstacle_item.clearance_class_no(), layer);

        }
        else if (obstacle_item instanceof freerouting.board.Via)
        {
            TileShape via_shape = ((freerouting.board.Via) obstacle_item).get_tree_shape_on_layer(this.search_tree, layer);
    ...
    private int check_ripup(MazeListElement p_list_element, Item p_obstacle_item, boolean p_door_is_small)
    {
        ...
        if (p_obstacle_item instanceof freerouting.board.Trace)
        {
            freerouting.board.Trace obstacle_trace = (freerouting.board.Trace) p_obstacle_item;
        ...
        else if (p_obstacle_item instanceof freerouting.board.Via)
        {
            ...
            {
                if (!(curr_contact instanceof freerouting.board.Trace) || curr_contact.is_user_fixed())
                {
                    return -1;
                }
                ++contact_count;
                freerouting.board.Trace obstacle_trace = (freerouting.board.Trace) curr_contact;
     ...
     */
    private static double calc_fanout_via_ripup_cost_factor(freerouting.board.Trace p_trace)
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            if (curr_trace_contact instanceof freerouting.board.Pin && curr_trace_contact.first_layer() == curr_trace_contact.last_layer())
            {
                protect_fanout_via = true;
            }
            else if (curr_trace_contact instanceof PolylineTrace && curr_trace_contact.get_fixed_state() == freerouting.board.FixedState.SHOVE_FIXED)
            {
    ...
    private double check_neck_down_at_dest_pin(CompleteExpansionRoom p_room)
    {
        ...
        {
            if (curr_target_door.item instanceof freerouting.board.Pin)
            {
                return ((freerouting.board.Pin) curr_target_door.item).get_trace_neckdown_halfwidth(p_room.get_layer());
            }
        }
        return 0;
    }
