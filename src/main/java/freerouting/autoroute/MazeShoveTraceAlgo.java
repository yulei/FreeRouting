...
public class MazeShoveTraceAlgo
{
            ...
            boolean p_shove_to_the_left, Collection<DoorSection> p_to_door_list)
    {
        ...
        if (!(p_obstacle_room.get_item() instanceof freerouting.board.PolylineTrace))
        {
            return true;
        }
        freerouting.board.PolylineTrace obstacle_trace = (freerouting.board.PolylineTrace)p_obstacle_room.get_item();
        ...
        for (ExpansionDoor curr_door : room_doors)
        {
            ...
            else if (!segment_ist_point)
            {
                ...
                {
                    if (p_board.get_test_level() == freerouting.board.TestLevel.ALL_DEBUGGING_OUTPUT)
                    {
     ...
     */
    private static boolean end_points_matching(freerouting.board.PolylineTrace p_trace, Item p_from_item)
    {
        ...
        if (p_from_item instanceof freerouting.board.DrillItem)
        {
            Point from_center = ((freerouting.board.DrillItem) p_from_item).get_center();
            points_matching =  from_center.equals(p_trace.first_corner()) || from_center.equals(p_trace.last_corner());
        }
        else if (p_from_item instanceof freerouting.board.PolylineTrace)
        {
            freerouting.board.PolylineTrace from_trace = (freerouting.board.PolylineTrace) p_from_item;
